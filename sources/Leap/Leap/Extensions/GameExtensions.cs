...
public static class GameExtensions
{
    ...
    /// <param name="game">The Game instance to which the GraphicsCompositor will be added.</param>
    /// <returns>The newly configured GraphicsCompositor instance with enabled post-effects.</returns>
    public static GraphicsCompositor AddGraphicsCompositor(this IGame game)
    {
        var sceneSystem = game.Services.GetService<SceneSystem>();

        if (sceneSystem == null)
        {
            throw new InvalidOperationException("SceneSystem is not available in the game instance.");
        }
    ...
    /// </remarks>
    /// <exception cref="InvalidOperationException">Thrown if the GraphicsCompositor does not have any camera slots defined.</exception>
    public static Entity Add3DCamera(this IGame game, string? cameraName = CameraDefaults.MainCameraName, Vector3? initialPosition = null, Vector3? initialRotation = null, CameraProjectionMode projectionMode = CameraProjectionMode.Perspective)
    {
        var sceneSystem = game.Services.GetService<SceneSystem>();
        ...
        var cameraSlot = sceneSystem.GraphicsCompositor.Cameras[0];
    ...
    /// <para>The entity will be added to the game's root scene. You can customize the light properties by accessing the returned Entity object.</para>
    /// </remarks>
    public static Entity AddDirectionalLight(this IGame game, string? entityName = "Directional Light")
    {
        var sceneSystem = game.Services.GetService<SceneSystem>();
    ...
    /// The profiler's behaviour can be interacted with using various keyboard shortcuts as described in the <see cref="GameProfiler"/> class.
    /// </remarks>
    public static Entity AddProfiler(this IGame game, string? entityName = "Game Profiler")
    {
        var sceneSystem = game.Services.GetService<SceneSystem>();
